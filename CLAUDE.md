...
- **BullMQ queue depth monitoring**: BullMQ distributes jobs across three keys per queue: `bull:<queue>:prioritized` (sorted set), `bull:<queue>:wait` (list), and `bull:<queue>:active` (sorted set). To calculate true queue depth, sum `zcard prioritized + llen wait + zcard active`. The `prioritized` set holds jobs with explicit priority values—always check it when debugging stuck jobs. See `apps/worker/src/processors/synth-job.processor.ts:getQueueDepth()`.
